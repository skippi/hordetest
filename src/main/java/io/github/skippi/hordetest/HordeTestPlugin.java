...
public class HordeTestPlugin extends JavaPlugin implements Listener {
  ...
  private static void populateSugarCane(@NotNull Chunk chunk, @NotNull Random random) {
    ...
    List<Material> growBlocks =
        Arrays.asList(Material.SAND, Material.GRASS_BLOCK, Material.RED_SAND);
    int count = 0;
    for (int i = 0; i < 16; ++i) {
      for (int j = 0; j < 16; ++j) {
        if (count > 3) break;
        ...
        Block block =
            world.getHighestBlockAt(
                chunk.getBlock(i, 255, j).getLocation(), HeightMap.MOTION_BLOCKING_NO_LEAVES);
        ...
        for (int x = -1; x <= 1; ++x) {
          for (int z = -1; z <= 1; ++z) {
            if (0 <= i + x
                ...
                && 0 <= j + z
                && j + z < 16
                && chunk.getBlock(i + x, block.getY(), j + z).getType().equals(Material.WATER)) {
              hasWater = true;
              break;
            }
          }
  ...
  private static void populateCobweb(
      @NotNull Chunk chunk, @NotNull Random random, IntRange sizeRange) {
    int maxCount = 3;
    for (int i = 0; i < 16; ++i) {
      for (int k = 0; k < 16; ++k) {
        for (int j = 0; j < 96; ++j) {
          ...
          int veinSize =
              random.nextInt(sizeRange.getMaximumInteger() - sizeRange.getMinimumInteger())
                  + sizeRange.getMinimumInteger();
          ...
          while (!queue.isEmpty() && veinSize > 0) {
            ...
            if (!(currAbove.isPresent() && currAbove.get().getType().equals(Material.STONE)))
              continue;
            --veinSize;
  ...
  private static void populateExtraOre(
      @NotNull Chunk chunk,
  ...
  private static Optional<Block> getRelativeWithinChunk(Block block, int x, int y, int z) {
    ...
    if (!(0 <= relX && relX < 16 && 0 <= relY && relY < 256 && 0 <= relZ && relZ < 16))
        ...
        .add(
            new BlockPopulator() {
              ...
              public void populate(
                  @NotNull World world, @NotNull Random random, @NotNull Chunk source) {
